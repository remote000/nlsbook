...
V=rand(NUM_STATES,1)./10;
Psa=ones(NUM_STATES,NUM_STATES,2)./NUM_STATES;
Psacnt=zeros(NUM_STATES,NUM_STATES,2);
Rcnt=zeros(NUM_STATES,1);
Rsum=zeros(NUM_STATES,1);
Rs=zeros(NUM_STATES,1);
streak=0;
%%%% END YOUR CODE %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while num_failures<max_failures && streak<NO_LEARNING_THRESHOLD
...
Psacnt(state,new_state,action)=Psacnt(state,new_state,action)+1;
Rcnt(new_state)=Rcnt(new_state)+1;
Rsum(new_state)=Rsum(new_state)+R;
  ...
  if (new_state==NUM_STATES)
    ...
    % component (and thus keep it at the initialized uniform distribution).
        for a=[1,2]
            for s=1:NUM_STATES
                sa_dnorm=sum(Psacnt(s,:,a));
                if(sa_dnorm>0)
                    Psa(s,:,a)=Psacnt(s,:,a)./sa_dnorm;
                end
            end
        end
        for s=1:NUM_STATES
...
V_old=ones(NUM_STATES,1);
itcnt=0;
while max(abs(V-V_old))>TOLERANCE
    V_old=V;
    for s=1:NUM_STATES
        V(s)=Rs(s)+GAMMA*max([Psa(s,:,1)*V_old,Psa(s,:,2)*V_old]);
    end
    itcnt=itcnt+1;
end
